...
let pair2str ((i, j), (w, dir)): string = 
  let rec pair2str' i j w dir result =
    if w < 0 then (result ^ "] ")
    else pair2str' (i+directions.(dir).(0)) (j+directions.(dir).(1)) (w-1) dir (result ^ (sprintf "(%i,%i); " i j))
...
type pairExist = (int * int, bool) Hashtbl.t
...
let pairs_exist (pair_list: (int*int) list) (pe: pairExist):bool =
  ...
  let rec pairs_exist' pair_list' = 
    ...
    | [] -> false
    | p::rest -> if pair_exist p then true else pairs_exist' rest
...
let cmdopt_total_nets :int ref = ref 3
...
let nets = M.random_nets 10 (M.pair_hash [] true) 4;;
print_endline (M.nets2str nets);; *)
main ();;
